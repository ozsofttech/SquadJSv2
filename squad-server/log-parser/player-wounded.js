export default {
  regex:
    /^\[([0-9.:-]+)]\[([ 0-9]*)]LogSquadTrace: \[DedicatedServer](?:ASQSoldier::)?Wound\(\): Player:(.+) KillingDamage=(?:-)*([0-9.]+) from ([A-z_0-9]+) \(Online IDs: (?:EOS: )?([0-9a-f]{32}|INVALID)(?: steam: )?([0-9]{17})? \| Controller ID: (.+)\) caused by ([A-z_0-9-]+)_C/,
