...
export default class SquadRcon extends Rcon {
  processChatPacket(decodedPacket) {
    const matchChat = decodedPacket.body.match(
      /\[(ChatAll|ChatTeam|ChatSquad|ChatAdmin)] \[Online IDs:EOS: ([0-9a-f]{32}) steam: ([0-9]{17})] (.+?) : (.*)/
    );
    if (matchChat) {
    ...
    const matchPossessedAdminCam = decodedPacket.body.match(
      /\[Online Ids:EOS: ([0-9a-f]{32}) steam: ([0-9]{17})] (.+?) has possessed admin camera./
    );
    ...
    const matchUnpossessedAdminCam = decodedPacket.body.match(
      /\[Online IDs:EOS: ([0-9a-f]{32}) steam: ([0-9]{17})] (.+?) has unpossessed admin camera./
    );
    ...
    const matchKick = decodedPacket.body.match(
      /Kicked player ([0-9]+)\. \[Online IDs= EOS: ([0-9a-f]{32}) steam: ([0-9]{17})] (.*)/
    );
    if (matchKick) {
    ...
    const matchSqCreated = decodedPacket.body.match(
      /(.+) \(Online IDs: EOS: ([0-9a-f]{32}) steam: ([0-9]{17})\) has created Squad (\d+) \(Squad Name: (.+)\) on (.+)/
    );
    if (matchSqCreated) {
    ...
    const matchBan = decodedPacket.body.match(
      /Banned player ([0-9]+)\. \[Online IDs= EOS: ([0-9a-f]{32}) steam: ([0-9]{17})] (.*) for interval (.*)/
    );
    if (matchBan) {
  ...
  async getListPlayers() {
    ...
    for (const line of response.split('\n')) {
      const match = line.match(
        /ID: ([0-9]+) \| Online IDs: EOS: ([0-9a-f]{32}) steam: ([0-9]{17}) \| Name: (.+) \| Team ID: ([0-9]+) \| Squad ID: ([0-9]+|N\/A) \| Is Leader: (True|False) \| Role: ([A-Za-z0-9_]*)\b/
      );
      if (!match) continue;
  ...
  async getSquads() {
    ...
    for (const line of responseSquad.split('\n')) {
      const match = line.match(
        /ID: ([0-9]+) \| Name: (.+) \| Size: ([0-9]+) \| Locked: (True|False) \| Creator Name: (.+) \| Creator Online IDs: EOS: ([0-9a-f]{32}) steam: ([0-9]{17})/
      );
